...
**纳入文献：** 16篇（最终，经方案A严格筛选；#79 Ding 2021已排除E2）
...
projects/paper-02/
├── 03-screen/
│   └── 数据_5_第三轮全文筛选.xlsx        ← 筛选流程（109→16篇）
├── 04-extract/
│   ├── 数据_7_数据提取表_v1.xlsx         ← 核心数据（16篇，rows 4–19）
...
| 18 | #104 | Ursache 2016 | DTI | 107 | early-school | USA | positive | p=.001 |
| 19 | #109 | Lange 2010 | sMRI | 309 | mixed | UK | positive | VIQ +14–15pts, p<.00001 |
...
| 排除（E2，教育无独立效应） | 93篇 |
| 正向关联 | 14篇（88%） |
| 负向/混合 | 2篇（#5 Wienke ERP负向；#75 Shephard fMRI负向） |
| 样本量范围 | N=26–373 |
| 年龄范围 | 出生→15岁（重点0–8岁） |
| 国家数 | 10+（USA/UK/Germany/Canada/Spain/China/India/Norway等） |
| 模态分布 | sMRI×5, fMRI×3, DTI×3, ERP×2, rsEEG×2, fNIRS×1 |
...
1. **#5 Wienke 2024**：唯一负向ERP结果，需在3.3节单独讨论（移民背景+低教育的交互效应）
2. **#13 Brito 2020**：FDR边界问题，Results中加注"未通过FDR校正，原始p=0.025"
3. **#20 Konrad 2024**：效应是调节效应（interaction），不是主效应——"母亲教育调节海马→认知关联"
4. **#75 Shephard 2019**：fMRI负向结果（高教育→更弱connectivity，D3b=negative），需解释机制
...
1. **Table 1特征表**：16篇完整特征，CC生成后请oo核查NOS评分列
2. **PRISMA流程图数字**：109→（第一轮排除X篇）→（第二轮排除X篇）→（第三轮排除93篇）→16篇，请CC从筛选表提取各轮数字
3. **#75 Shephard 2019 效应方向**：D3b=negative（高教育→更弱fMRI connectivity）——Discussion需解释，请oo提供机制解读
